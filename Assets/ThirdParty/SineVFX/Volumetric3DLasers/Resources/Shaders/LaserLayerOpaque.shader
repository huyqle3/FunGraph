...
Shader "Sine VFX/V3DLasers/LaserLayerOpaque" {
    Properties {
        ...
        _OpacityStartPosition ("Opacity Start Position", Float ) = 4
        _OpacityStartShape ("Opacity Start Shape", Range(0, 10)) = 10
        _OpacityStartPower ("Opacity Start Power", Range(0, 1)) = 0.5
    ...
    SubShader {
        ...
        Pass {
            ...
            uniform float4 _FinalOuterColor;
            uniform float _OpacityStartPosition;
            uniform float _OpacityStartShape;
            uniform float _OpacityStartPower;
            uniform float _MaxDist;
            uniform float _ShapeConeForm;
            uniform float _Noise01Add;
            uniform float _FinalSize;
            uniform float _ImpactProgress;
            uniform float _ImpactPower;
            uniform fixed _InvertColors;
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : COLOR {
                ...
                float Noise01 = saturate((_Noise01_var.r+_Noise01Add));
                float node_348 = 0.0;
                float node_2034 = (0.75+_OpacityStartPosition);
                float ShapeForOpacity = (pow((node_348 + ( (clamp(((1.0 - node_3971)+_OpacityStartPosition),node_348,node_2034) - node_348) * (1.0 - node_348) ) / (node_2034 - node_348)),_OpacityStartShape)*_OpacityStartPower);
                float HackedOpacity = saturate((lerp((node_4345_if_leA*1.0)+(node_4345_if_leB*node_9078),node_9078,node_4345_if_leA*node_4345_if_leB)*((_OpacityRemap1 + ( (_OpacityCutoff_var.r - node_2660) * (_OpacityRemap2 - _OpacityRemap1) ) / (1.0 - node_2660))-node_616)*(Noise01+ShapeForOpacity)));
                clip(HackedOpacity - 0.5);
////// Lighting:
////// Emissive:
                float node_9603 = Noise01;
                float node_2322 = (1.0 - node_9603);
                float3 emissive = (lerp((_FinalColor.rgb*_FinalPower*node_9603),(_FinalPower*lerp(_FinalColor.rgb,_FinalOuterColor.rgb,saturate((1.0 - _ImpactProgress)))*saturate(node_2322)*_ImpactPower),lerp( isFrontFace, (1.0 - isFrontFace), _InvertColors ))*_GammaLinear);
